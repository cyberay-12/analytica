        ...
        <div className="px-6 pt-4">
            <div className="grid grid-cols-4 md:grid-cols-12 gap-3 mb-2">
                <div className="col-span-4">       
                        <div>
                            <MainCard number={data?.stats?.total_award} title="Total Awards" />
                        </div>
                        <div className="mt-3">
                            <SecCard number={data?.stats?.new_award} percent={<span className={data?.percentages?.year_percent > 0 ? 'text-green-600' : 'text-red-600'}>
                            {data?.percentages?.year_percent > 0 ? '▲' : '▼'} {data?.percentages?.year_percent}%
                            </span>} title={`New Awards in ${data?.stats?.max_year}`}/>
                        </div>
                </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-12 gap-3">
                ...
                <div className="col-span-3 h-96 border-t-6 border-green-600 bg-white rounded-[1vw] inset-shadow-xl shadow-xl">
                    <div className='grid grid-rows-7 h-full'>
                        <div className='row-span-1 font-[750] text-lg text-gray-700 w-full rounded-t-[1vw] align-middle pt-4 pl-7'>Awards per Mode</div>
                        <div className='row-span-6 h-full w-full'>
                        <Plotly 
                            data={[{
                                values: data?.charts?.per_mode_values,
                                labels: data?.charts?.per_mode_labels,
                                hovertemplate: '<b>Mode:</b> %{label} <br><b>Count:</b> %{value}<br><b>Percent: </b>%{percent}<extra></extra>',
                                hole: .6,
                                type: 'pie',
                                outsidetextfont: {color: 'transparent'},
                                marker: {colors: ['#01ac42', '#FFEB00']}
