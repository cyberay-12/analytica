...
import { faMoneyBills, faMoneyBill, faMagnifyingGlass} from "@fortawesome/free-solid-svg-icons";
    ...
    <div className="px-6 pt-4">
      <div className="grid grid-cols-3 sm:grid-cols-6 md:grid-cols-12 gap-3 mb-2">
        <div className='col-span-3'>
          <MainCard icon={<FontAwesomeIcon className="text-green-600 text-3xl" icon={faMagnifyingGlass} />} number={data?.stats?.total_study} title="Total Studies" />
        </div>
        <div className='col-span-3'>
          <SecCard icon={<FontAwesomeIcon className="text-white text-3xl" icon={faCalendarCheck} />} number={data?.stats?.completed_studies} percent={<span>{data?.percentages?.complete_perc}%</span>} title="Completed Studies"/>
        </div>
        <div className='col-span-3'>
          <SecCard icon={<FontAwesomeIcon className="text-white text-3xl" icon={faCalendarMinus} />} number={data?.stats?.ongoing_studies} percent={<span>{data?.percentages?.ongoing_perc}%</span>} title="Ongoing Studies"/>
        </div>
        <div className='col-span-3'>
          <SecCard icon={<FontAwesomeIcon className="text-white text-3xl" icon={faCalendarPlus} />} number={data?.stats?.new_studies} percent={<span className={data?.percentages?.year_percent > 0 ? 'text-green-600' : 'text-red-600'}>
            {data?.percentages?.year_percent > 0 ? '▲' : '▼'} {data?.percentages?.year_percent}%
            </span>} title={`New Studies in ${data?.stats?.max_year}`}/>
        </div>
      </div>
    ...
    <div className='grid grid-cols-6 md:grid-cols-12 gap-2'>
        ...
        <div className='bg-linear-to-br from-white to-gray-100 flex flex-wrap h-96 rounded-[1vw] inset-shadow-xl shadow-xl'>
          ...
          <div className='grid grid-cols-5 gap-x-1'>
            <div className='bg-white inset-shadow-lg rounded-lg shadow-xl h-37 mt-2 mr-1 pt-4 pl-4 gap- col-span-5 md:col-span-3 overflow-hidden'>
              <div className='bg-green-500/70 rounded-lg h-12 w-16'>
                  <FontAwesomeIcon className="text-white text-3xl pt-2 pl-3" icon={faMoneyBills} />
              </div>
            ...
            <div className='bg-white inset-shadow-lg rounded-lg shadow-xl h-37 pt-4 pl-4 mt-2 mr-1 col-span-5 md:col-span-2 overflow-hidden'>
              <div className='bg-green-500/70 rounded-lg h-12 w-16'>
                  <FontAwesomeIcon className="text-white text-3xl pt-2 pl-3" icon={faMoneyBill} />
              </div>
              <div className='pt-2'>
      ...
      <div className='col-span-7'>
        <div className='border-t-6 border-green-600 bg-white inset-shadow-lg h-66 rounded-[1vw] shadow-xl'>
          <div className=' text-md font-(family-name: --font-inter) font-[550] w-full h-12 rounded-t-[1vw] align-middle pt-3 pl-4'>Initiated Programs per Year</div>
          <div className='pr-16'>
            <Plotly 
              data={[
                {
        ...
        <div className='border-t-6 border-green-600 bg-white inset-shadow-lg h-67 mt-2 rounded-[1vw] shadow-xl'>
          <div className='text-md w-full h-12 font-(family-name: --font-inter) font-[550] rounded-t-[1vw] align-middle pt-3 pl-4'>Approved Program Budget per Year</div>
          <div>
            <Plotly 
              data={[
                {
